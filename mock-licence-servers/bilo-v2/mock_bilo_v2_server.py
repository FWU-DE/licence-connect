...
def token():
    ...
    if (
        ...
        and client_id == config["bilo.v2.auth.clientId"]
        and client_secret == config["bilo.v2.auth.clientSecret"]
    ):
        return jsonify(
            {
...
@app.route(
    "/starbackend-v1/lookup/v1/USR__OOC_BY_LicenseConnectFWU/user/<user_id>",
    methods=["POST", "GET"],
)
def licences(user_id):
    if (
        request.headers.get("Authorization") == "Bearer mock_access_token"
        and user_id == "student.2"
    ):
        return jsonify(
            {
                "user": {
                ...
                "organizations": [
                    {
                        ...
                        "media": [],
                        "groups": [
                            {"id": "1", "name": "1", "group_type": "class", "media": []}
                        ],
                    }
                ],
            }
