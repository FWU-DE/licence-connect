...
def token():
    ...
    if (
        username == config["bilo.v1.auth.admin.username"]
        and password == config["bilo.v1.auth.admin.password"]
    ):
        return jsonify(
            {
...
def licences(user_id):
    if (
        request.headers.get("Authorization") == "Bearer " + mock_access_token_value
