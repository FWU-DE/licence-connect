...
def delete_test_assignments():
    ...
    test_assignments = [
        ...
        if (x["bundesland_id"] is not None and x["bundesland_id"].startswith(test_data_prefix))
        | (x["landkreis_id"] is not None and x["landkreis_id"].startswith(test_data_prefix))
...
def test_assign_media_only_landkreis_id_bad_request():
    response = client.post(
        "/admin/media-licence-assignment",
        headers={"x-api-key": admin_api_key},
        json={
            "landkreis_id": f"{test_data_prefix}123",
            "licenced_media": [{"id": "user-medium"}],
        },
    )
    assert response.status_code == 422
...
def test_assign_media_only_landkreis_and_schul_id_bad_request():
    response = client.post(
        "/admin/media-licence-assignment",
        headers={"x-api-key": admin_api_key},
        json={
...
def test_assign_multiple_media_in_one_request_success():
    ...
    licenced_media_only_response = client.get(
        f"/licenced-media?bundesland_id={test_data_prefix}bundesland",
